				...
				if(!isset($types[$field_name]))//Doesn't exist in the database? Create it for the CRUD
				{
					...
					$field_info = (object)array(
						...
						'display_as' => isset($this->display_as[$field_name]) ?
												$this->display_as[$field_name] :
												ucfirst(str_replace("_"," ",$field_name)),
						'required'	=> is_array($this->required_fields) ? (in_array($field_name,$this->required_fields) ? true : false) : false,
						'extras'	=> $extras
					);
		...
		switch ($real_type) {
			...
			case 'upload_file':
				...
				else
				{
					...
					if($is_image)
					{
						$file_url_anchor .= ' class="image-thumbnail"><img src="'.$file_url.'" height="50" class="gridImagePrev">';
					}
					else
					{
...
class Grocery_CRUD extends grocery_CRUD_States
{
	...
	public function __construct() {
		$this->default_language_path				= APPPATH.'views/gcrud/languages';
		$this->default_config_path					= APPPATH.'views/gcrud/config';
		$this->default_assets_path					= APPPATH.'views/gcrud';
	}
